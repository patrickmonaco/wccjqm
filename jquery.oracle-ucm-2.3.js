 ...
 * 2.1 : Patrick Monaco: Handle calls for thesaurus
 * 2.2 : PMO: Add get_profile
 * 2.3 : PMO: add trackers
 */
(function($) {
	$.ucm = {
		...
		// PMO POur thesaurus
		getSchemaValues2 : function(view, callback) {
			var schemaData = {
		...
		/**
		 * perform  Collection Display for browsing folders by CollectionId
		 */
		...
		/**
		 * perform  Collection Display for browsing folders by Path
		 */
		collectionDisplayByPath : function(collectionPath, callback) {
			var queryTextData = {
				IdcService: "COLLECTION_DISPLAY",
				hasCollectionPath : "true",
                                dCollectionPath : collectionPath
			}
			$.ucm.executeService(queryTextData, callback);
		},
		...
		docTracker : function(callback) {
			var queryTextData = {
				IdcService: "SCT_GET_DOCUMENT_INFO_ADMIN",
				defaultQuery : "qSctrTopDocs",
				QueryName : "qSctrTopDocs",
				FromDate : "5/26/2015",
				ToDate : "6/2/2015",
				NumDocs : "250",
				RowsPerPage : "25", 
				DocCriteriaValue : "No Drill Available",
				SctFmtFromDate : "{ts '2015-05-26 00:00:00.000'}",
				SctFmtToDate : "{ts '2015-06-04 00:00:00.000'}",
				StartRow : "0"
			}
			$.ucm.executeService(queryTextData, callback);
		},
		...
		/**
		 * perfom displaying Folio Node  
		 */
		loadFolioNode : function(id, callback) {
			var queryTextData = {
				IdcService: "LOAD_FOLIO_NODE",
		 ...
		 * Get Doc profiles  
		 */
		getDocProfiles : function(callback) {
			var queryTextData = {
				IdcService: "GET_DOCPROFILES"	
			}
			$.ucm.executeService(queryTextData, callback);
		},
		 ...
		 */
		quickSearch : function(quickSearchText, exts, nbdoc, callback) {
			// PMO
			var extraFilters = '';
			var qs='';
			//alert('js' + exts);
			...
			if (exts != '') {
				...
				extraFilters = extraFilters + exts;
			}
			if (quickSearchText==''){
                            qs='';
			  } 
			  ...
			  {
			    qs= '<qsch>' + quickSearchText + '</qsch>';
			  }		
			var queryTextData = {
				IdcService: "GET_SEARCH_RESULTS",
				QueryText : qs + extraFilters,
				ResultCount : nbdoc,
				SortField : "dInDate",
				SortOrder : "Desc",
				StartRow: "1"
		...
		/**
		 * get values from a thesaurus
		 */
		thes : function(rel, dparent, callback) {
			if (rel=='top'){
			   var docInfoData = {
				IdcService: "GET_THES_TOP"	
			   }
			} else if (rel=='bt'){
			   var docInfoData = {
				IdcService: "GET_THES_BT",
				THP_PHRASE: dparent	
			   }

			} else if (rel=='rt'){
			   var docInfoData = {
				IdcService: "GET_THES_RT",
				THP_PHRASE: dparent	
